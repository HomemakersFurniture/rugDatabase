...
function DesignDetail() {
  const { collectionName, designId } = useParams();
    ...
    fetch(dataPath)
      ...
      .then(jsonData => {
        const filtered = jsonData.filter(
          item =>
            item['Collection Name'] === decodeURIComponent(collectionName) &&
            item['Design ID'] === decodeURIComponent(designId)
        );
        setData(filtered);
        setLoading(false);
      })
  ...
  const hasMultipleColors = colorData.length > 1;

  // Filter data by selected color (or show all if single color) and apply sorting
  const filteredData = useMemo(() => {
    let filtered;
    if (hasMultipleColors) {
      if (!selectedColor) return [];
      filtered = data.filter(item => item['Primary Color'] === selectedColor);
    } else {
      filtered = [...data];
    }
    ...
    return filtered;
  }, [data, selectedColor, sortConfig, hasMultipleColors]);

  // Get design info from first item
  const designInfo = useMemo(() => {
    if (data.length === 0) return { vendor: '', collectionName: '', designId: '' };
    return {
      vendor: data[0]['Vendor'] || 'Unknown',
      collectionName: data[0]['Collection Name'] || 'Unknown',
      designId: data[0]['Design ID'] || 'Unknown',
    };
  }, [data]);
      ...
      <table className="detail-table">
        ...
        <tbody>
          {filteredData.map((item, index) => {
            ...
            const isSku = productId === undefined || productId === null || productId === '';
            return (
              <tr key={rowKey} className={isSku ? 'sku-row' : undefined}>
